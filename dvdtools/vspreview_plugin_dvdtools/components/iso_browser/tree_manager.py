...
class ISOTreeManager:
    ...
    def _update_outputs(self, title_idx: int, angle: int | None) -> None:
        ...
        if (info := self.parent.title_info.get((title_idx, angle))) and info["chapters"]:
            ...
            try:
                chapter_frames = [int(frame) for frame in info["chapters"] if frame > 0]
    ...
    def _update_info_label(self, info: dict[str, Any]) -> None:
    ...
    def _populate_chapters_tree(self, info: dict[str, Any]) -> None:
